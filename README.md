...
The GitHub Pages frontend includes a private, local-only CV workspace. Upload a PDF resume — parsed locally with a vendored copy of Mozilla pdf.js, no network call — or paste/import a plain-text, Markdown or HTML profile, save it in the browser, and compare it with current roles. It keeps resume text-readiness, evidence match, hard gaps and cover-letter requirement detection as separate signals. The match is an auditable evidence signal, not an AI hiring decision; the resume score is a heuristic, not an employer ATS score. Cover-letter drafts are gated by the posting text and exact profile evidence, and never fabricate experience, metrics or skills. See [`docs/cv-evidence-and-ats-workflow.md`](docs/cv-evidence-and-ats-workflow.md).
...
- All 60 companies from the RCV registry, shown in the Company directory; 72 source connector identities are currently registered in the backend, with structured ATS feeds preferred over Firecrawl fallback
