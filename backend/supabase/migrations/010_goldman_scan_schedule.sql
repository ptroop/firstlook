-- Goldman Sachs uses a public, paginated India board. Reconcile the complete
-- inventory every 30 minutes; the Edge Function still hydrates only its due batch.
...
begin
  ...
  from cron.job
  where jobname = 'first-look-goldman-reconcile';
...
select cron.schedule(
  'first-look-goldman-reconcile',
  '9,39 * * * *',
  $$select public.invoke_first_look_scan('goldman-reconcile', 'reconcile', 60000);$$
);
