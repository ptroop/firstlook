...
begin
  select jobid into existing_job_id from cron.job where jobname = 'first-look-razorpay-watch';
  if existing_job_id is not null then perform cron.unschedule(existing_job_id); end if;
  select jobid into existing_job_id from cron.job where jobname = 'first-look-razorpay-reconcile';
  if existing_job_id is not null then perform cron.unschedule(existing_job_id); end if;
end;
$$;

select cron.schedule('first-look-razorpay-watch', '26,56 * * * *', $$select public.invoke_first_look_scan('razorpay-watch', 'watch', 60000);$$);
select cron.schedule('first-look-razorpay-reconcile', '29 */2 * * *', $$select public.invoke_first_look_scan('razorpay-reconcile', 'reconcile', 120000);$$);
